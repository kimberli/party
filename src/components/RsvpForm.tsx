...
const RESPONSE_OPTIONS = [
  ...
  "maybe, i'll come if i'm not skiing that weekend",
  "no,  i can't make it but i want to see who else is going",
] as const;

type ResponseOption = (typeof RESPONSE_OPTIONS)[number] | 'other' | null;

export default function RsvpForm(): JSX.Element | null {
  const { name, isValidated, error, lastResponse, loadGuestInfo, submitRsvp } = useApp();
  ...
  return (
    <div className="w-full max-w-xl mx-auto rounded bg-white p-4">
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {status && (
          <div className={`mt-4 text-center text-sm text-gray-600 ${status}`}>{status}</div>
        )}
        {error && (
          <div className={`mt-4 text-center text-sm text-orange-600 ${error}`}>{error}</div>
        )}
      </form>
    </div>
  );
